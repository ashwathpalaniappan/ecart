...
def home(request):
    ...
    if len(context['products']) == 0:
        res = requests.get('http://localhost:5000/getProducts')
        products = res.json()
...
def user(request):
    ...
    if request.method == 'POST':
        ...
        'state': state,
        'pincode': pincode,
        'order_items': str(context['cart_items']),
...
def payment(request):
    if request.method == 'POST':
        ...
        json_data = request.session['order']
        r = requests.post(url=api_url, json=json_data)

        #send mail
        api_url2 = 'http://localhost:5000/mailing'
        json_data2 = {
            'name': json_data['name'],
